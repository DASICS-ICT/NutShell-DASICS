...
class CSR(implicit val p: NutCoreConfig) extends NutCoreModule with HasCSRConst{
  ...
  val misaInitVal = getMisaMxl(2) | extList.foldLeft(0.U)((sum, i) => sum | getMisaExt(i)) //"h8000000000143105".U
  val misa = RegInit(UInt(XLEN.W), misaInitVal)
  // MXL = 2          | 0 | EXT = b 00 0000 0100 0011 0001 0000 0101
...
//  }
  ...
  if (!p.FPGAPlatform) {
    ...
    difftest.io.dsmbound0 := RegNext(dasicsSMainBoundLo)
    difftest.io.dsmbound1 := RegNext(dasicsSMainBoundHi)
  ...
  //  difftest.io.djbound0lo := RegNext(dasicsJumpBoundLoList(0))
  //  difftest.io.djbound0hi := RegNext(dasicsJumpBoundHiList(0))
  //  difftest.io.djbound1lo := RegNext(dasicsJumpBoundLoList(1))
  //  difftest.io.djbound1hi := RegNext(dasicsJumpBoundHiList(1))
  //  difftest.io.djbound2lo := RegNext(dasicsJumpBoundLoList(2))
  //  difftest.io.djbound2hi := RegNext(dasicsJumpBoundHiList(2))
  //  difftest.io.djbound3lo := RegNext(dasicsJumpBoundLoList(3))
  //  difftest.io.djbound3hi := RegNext(dasicsJumpBoundHiList(3))
